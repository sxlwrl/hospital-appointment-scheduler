...
describe('AuthService', () => {
  ...
  describe('Register', () => {
    it('Should create a new user if username and email are not taken', async () => {
      ...
      (mockPatientRepository.findByUsername as jest.Mock).mockResolvedValueOnce(
        null,
      );
      (mockPatientRepository.findByEmail as jest.Mock).mockResolvedValueOnce(
        null,
      );
      (bcrypt.hash as jest.Mock).mockResolvedValueOnce('hashed_password');
      (mockPatientRepository.create as jest.Mock).mockResolvedValueOnce(
        registerDto,
      );
      ...
      expect(mockPatientRepository.findByEmail).toHaveBeenCalledWith(
        'test@example.com',
      );
