...
describe('AvailService', () => {
  ...
  describe('findById', () => {
    it('Should return availability if it exists', async () => {
      const availability = {
        ...
        available_time: '14:30',
        duration: 30,
      };
      (mockAvailabilityRepository.findById as jest.Mock).mockResolvedValueOnce(
        availability,
      );
    ...
    it('Should return all availabilities', async () => {
      const availabilities = [
        ...
        },
        {
          ...
          available_time: '16:30',
          duration: 25,
        },
      ];
      (mockAvailabilityRepository.findAll as jest.Mock).mockResolvedValueOnce(
        availabilities,
      );
  ...
  describe('createAvail', () => {
    it('Should create a new availability if the doctor exists', async () => {
      ...
      (mockAvailabilityRepository.create as jest.Mock).mockResolvedValueOnce(
        createdAvail,
      );
    ...
    it('Should throw NotFoundError if doctor does not exist', async () => {
      ...
      await expect(service.createAvail(createDto)).rejects.toThrow(
        NotFoundError,
      );
  ...
  describe('updateAvail', () => {
    it('Should update an existing availability if the doctor exists', async () => {
      ...
      (mockAvailabilityRepository.update as jest.Mock).mockResolvedValueOnce(
        updatedAvail,
      );
      ...
      expect(mockAvailabilityRepository.findById).toHaveBeenCalledWith(1);
      expect(mockDoctorRepository.findById).toHaveBeenCalledWith(1);
      expect(mockAvailabilityRepository.update).toHaveBeenCalledWith(
        1,
        updateDto,
      );
    ...
    it('Should throw NotFoundError if availability does not exist', async () => {
      ...
      await expect(service.updateAvail(1, updateDto)).rejects.toThrow(
        NotFoundError,
      );
      expect(mockAvailabilityRepository.findById).toHaveBeenCalledWith(1);
    });
  ...
  describe('deleteAvail', () => {
    it('Should delete an existing availability', async () => {
      ...
      (mockAvailabilityRepository.delete as jest.Mock).mockResolvedValueOnce(
        undefined,
      );
